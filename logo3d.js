...
function initLogo3D(container) {
  ...
  // COLOUR-FAITHFUL MODE (data-use-svg-color="true"): the standard key/rim rig is tuned for
  // gold and metal and blows a saturated brand colour out to pastel (FELIX's red disc rendered
  // dusty rose). Dial the rig back for those logos so the customer's own colours survive.
  // ACES is the culprit for the pastel look — it desaturates bright saturated hues on the way
  // to white. Linear tone mapping plus a calmer rig keeps the red red and the cream cream.
  if (container.dataset.useSvgColor === 'true') {
    ...
    data.paths.forEach((path, pathIdx) => {
      ...
      if (useColorFromSvg && path.color) {
        ...
        pathFront = new THREE.MeshStandardMaterial({
          color: c, metalness: 0.0, roughness: 0.62, envMapIntensity: 0.3,
          polygonOffset: true, polygonOffsetFactor: -pathIdx, polygonOffsetUnits: -1
        });
        pathBevel = new THREE.MeshStandardMaterial({
          color: darken('#' + c.toString(16).padStart(6,'0'), 0.12),
          metalness: 0.1, roughness: 0.5, envMapIntensity: 0.3,
          polygonOffset: true, polygonOffsetFactor: -pathIdx, polygonOffsetUnits: -1
        });
      }
